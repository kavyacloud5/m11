...
router.post('/exhibitions', authMiddleware, async (req, res) => {
  try {
    const { id, title, dateRange, description, imageUrl, category } =
      req.body;
    await executeQuery(
...
router.get('/collectables', async (_req, res) => {
  try {
    const result = await executeQuery(
      'SELECT * FROM collectables ORDER BY id'
    );
    res.json(result.rows);
  } catch (_error) {
...
router.post('/collectables', authMiddleware, async (req, res) => {
  try {
    const { id, name, price, category, imageUrl, description, inStock } =
      req.body;
    await executeQuery(
...
router.delete(
  ...
  async (req, res) => {
    try {
      const { id } = req.params;
      await executeQuery('DELETE FROM collectables WHERE id = $1', [
        id,
      ]);
...
router.get('/shop-orders', async (_req, res) => {
  try {
    const result = await executeQuery(
      'SELECT * FROM shop_orders ORDER BY timestamp DESC'
    );
...
router.get('/gallery-images', async (_req, res) => {
  try {
    const result = await executeQuery(
...
router.get('/press-releases', async (_req, res) => {
  try {
    const result = await executeQuery(
      'SELECT * FROM press_releases ORDER BY date DESC'
    );
    res.json(result.rows);
  } catch (_error) {
